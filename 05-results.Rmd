...
Another question is that: does conversion rate keep constant between popular items and the others? Or, does people are more likely to buy the stuff which they click a lot? To do this, first we find out there are 4186 kind of items are bought. We divide them into 9 groups according to their clicking times, and calculate their conversion = buy/clicking. Here we get Fig 6 below:

```{r}
# 热门商品转化率低
tmp<-filter(sample_data,Behavior_type=='pv')
frequence_categories = names(sort(table(tmp$Category_ID),decreasing=TRUE))
conversion_Categories = rep(0,9)

sorting_conversion <- function(gap){
  for(i in c(1:9)){
    s = 1+gap*(i-1)
    t =  gap*i
...
fig6<-sorting_conversion(450)
fig7<-sorting_conversion(100)
fig6+fig7
```
